...
def autograd_assert(*args, **kwargs):
    ...
    mxnet.minpy.JITContext().mark_as_output(output)
...
def test_argnum():
    ...
    autograd_assert(
        a, b, True, argnum=[0, 1], func=f_with_mode, grad_func=f_add_grad)
    autograd_assert(
        a, b, False, argnum=[0, 1], func=f_with_mode, grad_func=f_mul_grad)
